---
title: "Feature Selection Framework - irrigation scenarios"
...
```{r}
load(here::here("data", "rf_wue_by_clim.RDS"))
df_wy <- annual_wy
df_wy0 <- r_wue_eco_0 %>% ungroup() 
df_wy2 <- r_wue_eco_2 %>% ungroup() 
...
summarize_data(r_wue_eco_0)
summarize_data(r_wue_eco_2)
```
...
```{r describe_strata}
strata <- r_wue_eco_0 %>%
  dplyr::select(c(scen, irr, rain)) %>%
  group_by(scen) %>% 
  summarize_if(is.numeric, mean) %>% 
  dplyr::arrange(scen) 

DT::datatable(strata,
              caption = "Summary of 5 irrigation scenarios total input during drought and rain input")
```
in the table, the irrigation and rain show average annual input during drought 2012-2015
...
```{r get_predictors}
# Save data frames of predictor variables for pre-drought WUE
 r_wue_eco_0 <- r_wue_eco_0 %>% ungroup() 
r_wue_eco_2 <- r_wue_eco_2 %>% ungroup() 

df_wy0_num_preds <- ungroup(r_wue_eco_0) %>% 
  select(!response & where(is.numeric)) 

df_wy0_fpreds <- r_wue_eco_0 %>% 
  select(!response & where(is.factor)) %>% 
  colnames()

# Save data frames of predictor variables for during drought
df_wy2_num_preds <- ungroup(r_wue_eco_2) %>% 
  select(!response & where(is.numeric)) 

df_wy2_fpreds <- r_wue_eco_2 %>% 
  select(!response & where(is.factor)) %>% 
  colnames()
```
...
#pref_order0 <- c("rain", "rz_storage", "trans", "evap","irr","sat_def","mean_cpool","veg","mean_dailyCstore","psn")
#pref_order2 <- c("rain", "rz_storage", "trans", "evap","irr","sat_def","mean_cpool","veg","mean_dailyCstore","psn")
```
...
imp0 <- train(x = r_wue_eco_0 %>% select(!response),
              y = r_wue_eco_0$response,
              method = 'rf',
              importance = TRUE,
              replace = TRUE,
...
# if the chunk above was used to select, change this to if statement 
pref_order0 <- if(exists('pref_order0')==F){
  varImp(imp0$finalModel, scale = FALSE) %>% 
  arrange(-Overall) %>% 
  rownames()
}else{
  pref_order0
...
# Find preliminary importance using random forest
imp2 <- train(x = r_wue_eco_2 %>% select(!response),
              y = r_wue_eco_2$response,
              method = 'rf',
              importance = TRUE,
              replace = TRUE,
              trControl = trainControl(method = "none", seed = 4326))
...
# Set preference order based on variable importance
pref_order2 <- if(exists('pref_order2')==F){
  varImp(imp2$finalModel, scale = FALSE) %>% 
  arrange(-Overall) %>% 
  rownames()
}else{
  pref_order2
...
wy0_vif <- remove_vif(select(df_wy0_num_preds,
                             -trans_reduc_perc, -rain), 
                      vif.threshold = vif.threshold, pref_order0)$selected.variables

wy0_cor <- remove_cor(select(df_wy0_num_preds,
                             -trans_reduc_perc, -rain), 
                      cor.threshold = cor.threshold, pref_order0)$selected.variables

wy0_select_variables <- intersect(wy0_vif, wy0_cor)

wy2_vif <- remove_vif(select(df_wy2_num_preds,
                             -trans_reduc_perc, -rain), 
                      vif.threshold = vif.threshold,
                      pref_order2)$selected.variables

wy2_cor <- remove_cor(select(df_wy2_num_preds,
                             -trans_reduc_perc, -rain), 
...
df_wy0_reduced <- r_wue_eco_0 %>% 
  select(c(response, all_of(df_wy0_preds)))

df_wy2_reduced <- r_wue_eco_2 %>% 
...
# change this so that if factor, don't select 
pred1_clim0 <- get_important_predictors(imp_wy0, 1, fpreds = df_wy0_fpreds)
pred2_clim0 <- get_important_predictors(imp_wy0, 2, fpreds = df_wy0_fpreds)
pred1_clim2 <- get_important_predictors(imp_wy2, 1, fpreds = df_wy2_fpreds)
pred2_clim2 <- get_important_predictors(imp_wy2, 2, fpreds = df_wy2_fpreds)

source(here::here("R", "create_binned_df.R"))

df_wy <- annual_wy %>% rename(response=npp)

df0_pred_binned <- create_binned_df(r_wue_eco_0)
df2_pred_binned <- create_binned_df(r_wue_eco_2)

```

```{r}
response_var="wue_eco"

ggplot(df0_pred_binned, aes(x = .data[[pred1_clim0]], y = .data[["response"]])) +
    geom_point() +
...
create_binned_plot(df0_pred_binned, clim_id = T)
create_binned_plot(df2_pred_binned, clim_id = T)
```
...
plotmo(rf_wy0, pmethod="partdep")
plotmo(rf_wy2, pmethod="partdep")
```
...
```{r}
varImpPlot(rf_wy0$finalModel, main = "Comparing variable importance measures for 2011")

varImpPlot(rf_wy2$finalModel, main = "Comparing variable importance measures for 2015")
```
