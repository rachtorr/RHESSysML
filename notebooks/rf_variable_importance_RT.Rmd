...
- clim 0 scenario = avg for 2011 (pre-drought) 
- clim 2 scenario = avg for 2015 (mid-drought)
...
```{r}
load(here::here("data", "rf_wue.RDS"))
df_wy <- annual_wy
df_wy0 <- r_wue_plant_2011
df_wy2 <- r_wue_plant_2015
```
...
summarize_data(r_wue_eco_drtyrs)
summarize_data(r_wue_plant_2011)
```
...
```{r describe_strata}
strata <- r_wue_eco_drtyrs %>%
  dplyr::filter(clim == "hist") %>% 
...
df_wy0_num_preds <- ungroup(r_wue_plant_2011) %>% 
  select(!response & where(is.numeric)) %>% 
  select(!wue_eco)

df_wy0_fpreds <- r_wue_plant_2011 %>% 
  select(!response & where(is.factor)) %>% 
  colnames()

# Save data frames of predictor variables for during drought
df_wy2_num_preds <- ungroup(r_wue_plant_2015) %>% 
  select(!response & where(is.numeric)) %>% 
  select(!wue_eco)

df_wy2_fpreds <- r_wue_plant_2015 %>% 
  select(!response & where(is.factor)) %>% 
  colnames()
```
...
imp0 <- train(x = r_wue_plant_2011 %>% select(!response),
              y = r_wue_plant_2011$response,
              method = 'rf',
              importance = TRUE,
              replace = TRUE,
...
# Find preliminary importance using random forest
imp2 <- train(x = r_wue_plant_2015 %>% select(!response),
              y = r_wue_plant_2015$response,
              method = 'rf',
              importance = TRUE,
              replace = TRUE,
              trControl = trainControl(method = "none", seed = 4326))
...
wy0_vif <- remove_vif(select(df_wy0_num_preds, -rain & -trans_reduc_perc), 
                      vif.threshold = vif.threshold, pref_order0[pref_order0 != 'rain' & pref_order0 !="trans_reduc_perc"])$selected.variables

wy0_cor <- remove_cor(select(df_wy0_num_preds, -rain & -trans_reduc_perc), 
                      cor.threshold = cor.threshold, pref_order0[pref_order0 != 'rain' & pref_order0 !="trans_reduc_perc"])$selected.variables

wy0_select_variables <- intersect(wy0_vif, wy0_cor)

wy2_vif <- remove_vif(select(df_wy0_num_preds, -rain & -trans_reduc_perc), 
                      vif.threshold = vif.threshold,
                      pref_order2[pref_order2 != 'rain' & pref_order2 != 'trans_reduc_perc'])$selected.variables

wy2_cor <- remove_cor(select(df_wy0_num_preds, -rain & -trans_reduc_perc), 
...
df_wy0_reduced <- r_wue_plant_2011 %>% 
  select(c(response, all_of(df_wy0_preds)))

df_wy2_reduced <- r_wue_plant_2015 %>% 
...
df_wy <- annual_wy %>% rename(response=wue_plant)
